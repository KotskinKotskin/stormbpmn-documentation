...
### Ретеншн-нуджи: погашение черновиков

Перед боевым запуском отправки ретеншн-нуджей (или после калибровочных прогонов) в базе могут накапливаться черновики писем, сгенерированные устаревшей версией движка. Эти черновики не удаляются автоматически: их срок действия (`valid_until`) ещё не истёк, а частотный фильтр исключает их владельцев из новой выборки, поэтому свежий нюдж, который мог бы отменить старый, не сгенерируется. Если включить отправку в таком состоянии, система разослёт всю накопленную пачку писем разом.

Для принудительного погашения всех неотправленных черновиков используйте административный эндпоинт:

```bash
curl -X POST http://<host>:8080/api/v1/admin/retention/discard-drafts \
  -H "Authorization: Bearer <token>"
```

::: tip Ответ синхронный
Эндпоинт возвращает количество погашенных черновиков сразу в теле ответа (HTTP 200), а не асинхронно через логи. Это позволяет администратору убедиться в результате операции до запуска боевой отправки.
:::

Доступ к эндпоинту требует полных административных прав (`@IsFullAccess`).
