...
### Отложенная активация пользователей (Pending Activation)

В новых версиях добавлена поддержка режима, при котором учётные записи создаются в статусе ожидания активации, если автоматическая выдача лицензий отключена. Это позволяет администратору контролировать количество активных пользователей и вручную активировать новые аккаунты.

**Настройка:**

Для включения режима отложенной активации установите переменную окружения:

| Параметр | Значение | Описание |
| --- | --- | --- |
| **createPendingEnterpriseUsers** | `true` | Включает создание учётных записей в статусе "ожидает активации" при первой попытке входа |

::: tip Поведение системы
При включённом режиме:
1. Пользователь пытается войти в систему.
2. Если учётная запись ещё не существует, она создаётся в базе данных.
3. Вход блокируется, и пользователю отображается сообщение: **«Учётная запись ждёт активации администратором»**.
4. Администратор видит список ожидающих активации пользователей в интерфейсе администрирования.
5. После активации администратором учётная запись занимает лицензию, и пользователь может войти.
:::

**Процесс активации администратором:**

1. Перейдите в раздел управления пользователями.
2. Найдите пользователей со статусом **«Ожидает активации»** (в интерфейсе отображается как `entities.pendingActivation`).
3. Нажмите кнопку **«Активировать»** (подсказка: *«Активировать (ожидает активации, займёт лицензию)»*). 
4. Убедитесь, что у вас достаточно свободных лицензий для активации пользователя.

::: warning Важно
Активация пользователя происходит только после явного действия администратора. До этого момента учётная запись существует в базе данных, но не может использоваться для входа в систему и не занимает лицензию.
:::
