...
Диаграммы, описания и пользователи хранятся в PostgreSQL. Лицензионный секрет
`LICENSE_KEY_V2` хранится вне базы данных: включите его защищённое хранилище в DR-процедуру
или предусмотрите отдельное восстановление секрета.

::: tip База данных, файлы и секреты
PostgreSQL содержит основные данные. S3-файлы (изображения, документы) также рекомендуется
включить в backup-стратегию, а внешние секреты, включая `LICENSE_KEY_V2`, необходимо сохранять
...
3. **Восстановите `LICENSE_KEY_V2`** из защищённого хранилища
4. **Сравните полный код активации** на `/admin-onboarding` с кодом, для которого выдан ключ;
   если он изменился, передайте новый код менеджеру и получите перевыпущенный ключ для
   восстановленной инсталляции
...
| 4.2 | **Получить ключи `LICENSE_KEY_V2`**          | Полный код каждой инсталляции — см. [«Активация инсталляции»](/operation/License.md#активация-инсталляции) |
...
| 8   | **Задать `LICENSE_KEY_V2`**                   | `LICENSE_KEY` хранить как секрет до конца окна отката |
| 9   | **Обновить тег в текущем контейнере**         | Изменить версию образа                       |
| 10  | **Перезапустить все экземпляры приложения**   | `docker-compose up -d`                       |
| 11  | **Проверить health checks и лицензию**        | Проверить `/admin-onboarding` и обычный вход |

### Критические обновления

::: danger Версия ≥ 6.6.XXXX — нужен новый `LICENSE_KEY_V2`
...
Каждой Enterprise-инсталляции нужен заново выпущенный ключ v2. `LICENSE_KEY`, лицензии v1 и
ранее выданные ключи больше не принимаются. До обновления или во время согласованного окна
скопируйте **полный** код активации с `/admin-onboarding` каждой инсталляции, включая тестовые
контуры, DR и клоны. После подстановки `LICENSE_KEY_V2` перезапустите все экземпляры приложения.
Подробности — [«Активация инсталляции»](/operation/License.md#активация-инсталляции).
:::
...
3. **Откатить версию контейнера**
    ...
    Перед запуском версии, использующей лицензию v1, восстановите в её конфигурации сохранённый
    `LICENSE_KEY`. Значение нельзя удалять из защищённого хранилища до конца окна отката.
