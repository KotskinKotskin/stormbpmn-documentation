...
:::warning
В облачной версии есть ограничения на использования API.
:::
...
[ 
  {
    ...
    "externalId": null,  //ID элемента во внешней системе.
    ...
    "type": "CLIENT" // тип элемента архитектуры. Возможные значения UNSPECIFIED, DOCUMENT, SYSTEM, COMMUNICATION, CLIENT, ENTITY, ACTION, OTHER
...
[ 
  {
    ...
    "description": "", // описание. Ожидается unescaped html, для отображения на UI.
    "externalUrl": null, // ссылка на внешнюю систему, можно прислать с плейсхолдером, например https://mywiki.corp.com/entity/${externalId}
    "type": "CLIENT", // тип элемент архитектуры. Возможные значения UNSPECIFIED, DOCUMENT, SYSTEM, COMMUNICATION, CLIENT, ENTITY, ACTION, OTHER.
    "status": "NEW", // статус элемента архитектуры. Возможные значения NEW, TRIAL, PRODUCTION, DECOMMISSIONING, ARCHIVE.
    "createdOn": "2024-05-02T17:19:41.459888", // дата создания.
    "updatedOn ": "2024-05-02T17:19:41.459888" // дата обновления.
    "updatedBy  ": "kotov@bpmn2.ru" // автор обновления.
  }
]
```
...
```js
{
    ...
    "you": true,
    "versionNumber": 4,
    "description": "<p>На вход то</p><p>&nbsp;</p><p>На выход это</p>",
    "public": true,
    "type": "BPMN",
    "tags": null,
    "totalApprovals": null,
    ...
    "outcommingLinks": [
        ...
        },
        {
            ...
            "toDiagramName": "1) Заправка ТС на АЗС",
            "fromDiagramName": "Процесс возмещения кпупного дохода по ОТС СДЕЛКАМ",
...
- {elementId} - идентификатор элемента на диаграмме. Обязателен.
Тело запроcа: 
```js
{
...
### Получение списка согласований пользователя по е-мейлу
...
- {status} - Статус согласования. Не обязателен. По-умолчанию вернутся все согласования. Возможные значения:  PENDING, ACCEPTED, DECLINED, COMPLETED
- {isApprover} - Признак согласующего. Не обязателен. По-умолчанию вернутся со всеми значениями. Возможные значения: true, false

Тело ответа:
```js
{
    "content": [  //размер этого массива говорит о количестве точных ответов (до предела SIZE). Если здесь будет больше 20, то нужно переключать page в query. В интерфейсах, которые используют интеграцию можно нарисовать кнопку "20+" это и так дофига.
        {
            "id": "867bc39c-9145-4eb5-b59f-00545a6a9a3f",
            "approverEmail": "r.aergaerg@sertgsertg.com",
            "approverName": null,
            "diagramId": "fbeb4c64-e172-4e09-9fa1-6e509a11ffde",
            "createdOn": "2025-01-23T17:44:27.323817",
            "createdOnStr": null,
            "approvalTime": null,
            "approvalTimeStr": null,
            "status": "PENDING",
            "createdBy": "r.aerhaer@aergaerg.com",
            "comment": null,
            "diagramVersion": "11",
            "diagramName": "Работа с подрядчиком на ЭТП. Верхний уровень",
            "direction": null,
            "importantComments": []
        }
    ],
