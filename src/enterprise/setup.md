...
Stormbpmn поставляется как docker-контейнер из приватного репозитория, который включается в себя и front, и back. Установка заключается в правильном указании .ENV-перменных внутрь контейнера, поднятии смежных контейнеров (если необходимо), и манупляции в административном интерфейсе. 
...
- Создайте пользователя с полными правами доступа к базе. Запоните логин и пароль.
...
::: tip
Вот и всё, минимальная установка готова! Вы сможете попасть в приложение через веб интерфейс по порту 8080.
:::
...
```
Теперь учетная запись является административной и все последующие учетные записи в этой команде тоже будут административными. Не используйте эту учетную запись в работе.
...
- **MINIO_ACCESSKEY** - название учетной записи с провами на создание бакетов и запись файлов.
- **MINIO_SECRETKEY** - пароль учетной записи с провами на создание бакетов и запись файлов.
...
Мы предоставляет метрики в формате Prometeus, [подробности о подключении и обеспечении резервного копирования](/support/README.md).
...
На текущий момент существует 2 варианта работы с почтой под разные задачи:
 ...
 - Установите Mautic на последий релиз версии v.4.xxx (например v.4.2.1) из [докера](https://hub.docker.com/r/mautic/mautic)
 ...
 - Создайте шаблоных красивых писем с использованием плейсхолдеров, вставляйте текст в скобках на места, куда система подставит актуальные значения
...
Будет отсылаться информация о изменении версии, о комментариях, о новой задаче на согласовании, о завершении согласований.
