...
| Разворачивание всех компонентов (PostgreSQL с расширениями pgvector/pg_trgm и др. — см. [требования](/install/README.md), MinIO, Redis, Nginx и т.д.) | Заказчик (по документации) |
| Запуск StormBPMN, первоначальная настройка | Заказчик (по документации) |
| Активация лицензии (полный код активации — Команда Storm — ключ v2) | Storm + Заказчик |
...
После установки получите на `/admin-onboarding` полный код активации без сокращения и передайте
его менеджеру. Полученный ключ задайте в `LICENSE_KEY_V2` и перезапустите приложение. Для
production, каждого тестового контура, DR-площадки и клона требуется отдельный ключ.
Существующие инсталляции могут временно продолжать работу с устаревшим `LICENSE_KEY` без
немедленной смены; из-за известного ограничения безопасности запланируйте их поэтапную миграцию
на v2.
...
| 0.4 | Согласование метода аутентификации (Keycloak/ADFS/OAuth2) | [Безопасность — OAuth2](/configure/security/) |
| 0.5 | Согласование хранения данных (PostgreSQL, MinIO) | Карточка ИС в реестре ИБ |
| 0.6 | Согласование интеграции с SIEM (Syslog) | [Безопасность — SIEM](/configure/security/) |
| 0.7 | Проверка Docker-образов на уязвимости (Cosign-подпись) | Отчёт сканирования |
| 0.8 | Согласование отключения простой аутентификации | [Чек-лист ИБ](/configure/security/) |
...
-   [Production-ready установка](/install/production/)
...
**Документация**: [Безопасность](/configure/security/) — OAuth2, Keycloak, ADFS, Claims, CORS, custom SSL, чек-лист.
...
-   [SIEM / Syslog аудит](/configure/security/)
...
**Результат**: бэкапы автоматизированы, restore протестирован, используемые лицензионные
секреты (`LICENSE_KEY_V2` и временно нужный `LICENSE_KEY`) сохраняются в защищённом хранилище,
а DR-процедуры включают проверку выбранного режима лицензии и доступа обычного пользователя.
...
После запуска используйте рецепт [«Пульс внедрения Storm»](./Adoption_pulse.md): SQL-запросы для аудитории, удержания, активности пользователей и живой базы процессов, с примерами графиков и настройкой BI-панели.
