...
::: tip storm-media — это не только голос
Тот же сервис извлекает текст из **документов-вложений AI-чата** (pdf с OCR, docx, doc и др.) — см.
[«Вложения файлов и импорт схем»](./file-attachments.md). Обработка документов идёт на **CPU**:
GPU нужен только локальному Whisper. Если голосовой ввод не нужен, а вложения нужны — storm-media
можно развернуть на машине без видеокарты.
:::
