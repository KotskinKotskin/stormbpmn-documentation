...
Разворачиваем **`Qwen/Qwen3-Next-80B-A3B-Instruct`**. Почему именно она — бенчмарк 8 моделей через
реальный чат Storm, таблица кандидатов под 96 ГБ и оценки скорости — на соседней странице
**[Выбор модели](./README.md)**. Здесь важно одно требование к любой модели, которую вы развернёте:
...
делать. Убедитесь, что у модели есть поддержка tool-calling и что для неё в vLLM есть **парсер тулов**
(`--tool-call-parser`; для Qwen-семейства — `hermes`). Без этого чат Storm не заработает.
:::
