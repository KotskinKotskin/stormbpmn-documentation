...
> 📦 **Разворачиваете StormBPMN с нуля?** Вам сюда. Идите по порядку: сначала **«Быстрый старт»**, затем **«Production-установка»** — пропускать этапы нельзя.
...
#### Совместное редактирование (опционально)

| Контейнер | CPU      | RAM    | Диск | Порты | Описание                                          |
| --------- | -------- | ------ | ---- | ----- | ------------------------------------------------- |
| **Redis** | 0.5 vCPU | 512 MB | 1 GB | 6379  | Pub/Sub и сессии совместного редактирования        |

::: tip Когда нужен Redis?
Redis **необходим** при развертывании нескольких экземпляров StormBPMN (multi-instance / HA). Для одного экземпляра данные хранятся в памяти приложения, Redis не требуется.
:::
...
#### Модуль симуляций (отдельная лицензия)

| Контейнер     | CPU    | RAM   | Диск  | Порты             | Описание                                                        |
| ------------- | ------ | ----- | ----- | ----------------- | --------------------------------------------------------------- |
| **Storm DES** | 4 vCPU | 16 GB | 20 GB | 8081 (внутренний) | Движок симуляций процессов — см. раздел [«Симуляции»](/simulation/) |

### Требования к PostgreSQL: расширения и кодировка

StormBPMN использует несколько расширений PostgreSQL. Перед установкой убедитесь, что они **доступны в вашей сборке БД**, а сама база создана в кодировке **UTF8**.

| Расширение / параметр | Зачем нужно | Обязательность |
|---|---|---|
| Кодировка **UTF8** | Корректное хранение текста, ICU-коллация и натуральная сортировка названий | Обязательно |
| **pgvector** (`vector`) | Быстрый универсальный (семантический) поиск по платформе; также используется базой знаний AI-чата. Нужен **всем**, не только при AI-модуле | Обязательно (см. предупреждение ниже) |
| **pg_trgm** | Подсказки и поиск пользователей по части имени/почты (триграммы) | Обязательно для автодополнения |
| **pgcrypto** | Криптографические функции | Обязательно |
| **uuid-ossp** | Генерация UUID | Обязательно |
| **hstore** | Хранилище «ключ-значение» для движка векторного поиска | Обязательно |

**Что создаётся автоматически, а что — вручную:**

- Расширения `vector`, `uuid-ossp`, `hstore`, `pgcrypto` создаются миграциями при первом запуске (`CREATE EXTENSION IF NOT EXISTS …`) — **при условии**, что соответствующие пакеты установлены в сборке PostgreSQL, а у пользователя БД есть право создавать расширения (суперпользователь или роль с нужными правами).
- Расширение **`pg_trgm`** приложение **не создаёт само**. Если его нет, миграции индексов автодополнения **молча пропускаются**, и поиск пользователей по части имени/почты работать не будет. Создайте его заранее.

::: warning Официальный образ postgres не содержит pgvector
Стандартный образ `postgres:17` включает contrib-модули (`pg_trgm`, `pgcrypto`, `uuid-ossp`, `hstore`), но **не содержит pgvector**. Для самостоятельного развёртывания используйте образ **`pgvector/pgvector:pg17`** — это тот же PostgreSQL со всеми стандартными модулями плюс pgvector. Для управляемых БД (DBaaS) заранее убедитесь, что провайдер предоставляет расширение `pgvector` и у пользователя есть право его создать.
:::

::: tip Образ pgvector можно взять из нашего реестра
Если внешние реестры (Docker Hub) закрыты, не обязательно тянуть `pgvector/pgvector:pg17` извне — мы зеркалим официальный образ в наш приватный реестр и **подписываем его той же подписью Cosign**, что и основной образ StormBPMN. Он доступен по **тем же** учётным данным `cr.selcloud.ru`:

```bash
docker pull cr.selcloud.ru/stormbpmn-enterprise/pgvector:pg17
```

Перенос в свой корпоративный реестр — теми же шагами, что и для основного образа (см. [Быстрый старт](/install/quickstart/QUICKSTART_MANUAL.md)). Проверка подписи — как у основного образа (см. [Changelog → «Проверка подписи образа (Cosign)»](/Changelog/README.md)).
:::

::: tip Полный набор одной командой
Если ставите расширения вручную — выполните в **базе приложения** от суперпользователя:
```sql
...
| **Redis**      | 6379         | TCP        | Совместное редактирование (multi-instance) |
