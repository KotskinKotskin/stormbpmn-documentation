...
В целях безопасности доступ к Swagger UI (интерфейс API-документации) защищён механизмом **Basic Auth**. При попытке открыть страницу документации браузер запросит логин и пароль.

::: tip Данные для входа
Используйте учётные данные администратора или выделенного технического пользователя, указанные в конфигурации приложения.
:::

### Сборка Docker-образа и защита секретов

При самостоятельной сборке Docker-образа (например, из исходного кода) критически важно исключить попадание файлов с секретами (`.env`) в итоговый образ.

1. **Используйте `.dockerignore`**: Убедитесь, что в корне проекта присутствует файл `.dockerignore`, который исключает файлы `.env` и `.env.*` из контекста сборки. Это предотвращает попадание секретов (например, `VITE_YANDEX_CLIENT_SECRET`) в слои образа.
2. **Разделение исходников и артефактов**: В процессе сборки статические файлы фронтенда (собранный `dist`) должны копироваться в отдельную директорию (например, `frontend-dist`), а не поверх исходников (`frontend/`). Это гарантирует, что в образ попадёт только готовая статика, а не исходный код с потенциальными конфигурационными файлами.

::: warning Уязвимость раскрытия секретов
Если `.env` файлы попадают в образ, они могут стать доступны через HTTP-запросы к корню приложения (например, `GET /.env`), если веб-сервер (Nginx) не настроен на явный запрет доступа к скрытым файлам. Использование `.dockerignore` и раздельных директорий сборки устраняет этот риск на этапе создания образа.
:::
