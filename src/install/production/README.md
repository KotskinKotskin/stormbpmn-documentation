...
-   [ ] **Лицензия v2** - ключ с привязкой к инсталляции (см. [Лицензирование](/operation/License.md))
...
::: tip Ключи подписи MCP-сервера
Ключ подписи MCP (Model Context Protocol) генерируется автоматически при первом старте и хранится в базе данных в зашифрованном виде — он переживает рестарты и общий для всех нод, отдельная настройка не требуется. При желании держать ключ во внешней секретнице задайте пару **MCP_OAUTH_JWK_PRIVATE** / **MCP_OAUTH_JWK_PUBLIC**.
:::
