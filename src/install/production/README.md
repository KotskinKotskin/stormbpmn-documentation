...
-   [ ] **Аудит-логирование** - сбор пользовательских действий
-   [ ] **Балансировка нагрузки** - для высокой доступности
-   [ ] **Масштабирование** - несколько экземпляров приложения


::: warning Безопасность веб-хуков Модульбанка
Для защиты от подделки уведомлений о платежах необходимо настроить переменную окружения **assm.app.modulbank.token**. Без неё проверка подписи отключена (fail-open), и любой отправитель может имитировать успешную оплату.

- **assm.app.modulbank.token**: токен для проверки SHA1-подписи веб-хуков. Если переменная не задана или пуста, проверка пропускается, но логируют предупреждения. В production-среде заполнение обязательно.
:::

::: warning Безопасность MCP-сервера
Для production-развёртывания с поддержкой MCP (Model Context Protocol) необходимо явно задать ключи шифрования, чтобы избежать генерации временных (ephemeral) ключей при каждом перезапуске, что приводит к потере сессий и невозможности аутентификации.

- **MCP_OAUTH_JWK_PRIVATE**: приватный ключ JWK.
- **MCP_OAUTH_JWK_PUBLIC**: публичный ключ JWK.

Для стендов и тестовых окружений можно использовать флаг **MCP_OAUTH_JWK_ALLOW_EPHEMERAL=true**, чтобы разрешить использование временных ключей.
:::
