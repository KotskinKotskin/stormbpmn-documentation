...
В Storm Enterprise административные права выдаются **команде**, а не отдельному пользователю. Все участники команды с флагом `full_access = true` автоматически получают права администратора.

Эта команда предназначена исключительно для администрирования системы — в ней должно быть **1–2 аккаунта** тех, кто обслуживает Storm.
...
3. **Активация административных прав**:
    ...
    > На этапе начальной установки в системе одна команда, поэтому запрос без `WHERE` корректен. Если позже появятся несколько команд — используйте `WHERE id = '...'`, чтобы не затронуть лишние команды.
...
Не добавляйте в эту команду рядовых сотрудников — они получат полные права администратора. Для работы пользователей создайте отдельную рабочую команду (см. раздел ниже).
:::
...
После создания административной команды необходимо создать отдельную команду для повседневной работы сотрудников.

| Команда             | Кто входит                        | `full_access` |
| ------------------- | --------------------------------- | ------------- |
| Административная    | 1–2 аккаунта администраторов Storm | `true`        |
| Рабочая             | Все остальные сотрудники           | `false`       |

### Пошаговая инструкция:

1. Войдите в систему под административным аккаунтом.
2. Перейдите по ссылке: `/app/team-settings/general`.
3. Создайте новую команду — это и будет рабочая команда для сотрудников.
4. Пригласите в неё пользователей.

Флаг `full_access` для рабочей команды по умолчанию `false` — дополнительных действий в базе данных не требуется.

:::tip Управление правами через SQL
Выдать права администратора конкретной команде:
```sql
