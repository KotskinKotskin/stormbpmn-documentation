...
Без ключа бэкенд может запуститься в заблокированном лицензионном режиме: владелец сможет
открыть `/admin-onboarding` и завершить активацию, но обычные пользователи не смогут работать.

1. Откройте `/admin-onboarding` и скопируйте **весь код активации без сокращения**.
...
4. Перезапустите контейнер, дождитесь успешных health checks, проверьте статус лицензии на
   `/admin-onboarding` и вход лицензированного обычного пользователя.
