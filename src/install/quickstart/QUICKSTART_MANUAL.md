...
3. Настройте порты для доступа к приложению: 8080 (старый интерфейс), 8081 (новый интерфейс)
   и 8082 (API)
...
Для новой Enterprise-инсталляции используйте отдельный ключ v2. Существующий `LICENSE_KEY`
временно поддерживается без немедленной замены, но лицензии v1 устарели и имеют известное
ограничение безопасности.

Используйте согласованный с менеджером образ с поддержкой v2. До активации не открывайте
установку обычным пользователям: пустой v2 выбирает legacy-путь, а отсутствие обоих ключей
не гарантирует блокировку доступа.

1. Откройте `/admin-onboarding` без входа при первой установке и скопируйте **весь код активации без сокращения**.
2. Передайте полный код менеджеру и получите ключ для этой инсталляции.
3. Добавьте ключ в переменную окружения **`LICENSE_KEY_V2`**. Если рядом задан `LICENSE_KEY`,
   непустой `LICENSE_KEY_V2` имеет исключительный приоритет; при ошибке v2 возврата на v1 нет.
4. Перезапустите контейнер, дождитесь успешных health checks, проверьте статус лицензии на
   `/admin-onboarding`. Затем создайте администратора по инструкции ниже, настройте
   пользователей и проверьте вход и нужные модули (включая DES) обычной учётной записью.

Для тестового контура, DR-площадки и каждого клона запросите отдельный ключ.
Согласуйте в ключе права AI и DES — в v2 они по умолчанию выключены; подробнее в
[разделе о формате v2](/operation/License.md#формат-v2-и-доступ-к-модулям).

::: tip Уже работающая инсталляция
Если Storm уже установлен и работает с `LICENSE_KEY`, код активации берётся на той же странице
`/admin-onboarding` без входа и без запуска мастера. Порядок перевода на v2 — в разделе
...
1. После успешной активации лицензии откройте `/admin-onboarding` и начните настройку.
2. На шаге **«Администратор»** укажите email и пароль первого администратора. Мастер создаст
   учётную запись и отдельную административную команду с полными правами.
3. Сохраните учётные данные в защищённом хранилище и завершите остальные шаги мастера.

Если административная команда уже существует, войдите под её участником. Повторное создание
первого администратора не допускается. При неверном v2 сначала восстановите лицензию:
создание администратора на заблокированной инсталляции недоступно. Не назначайте права всем
командам массовым SQL-обновлением.
...
Для настройки системы входа в приложение ознакомьтесь с [подробной инструкцией по обеспечению информационной безопасности](/configure/security/authorization.md) и выберите подходящий вариант для вашей организации.
...
1. **[Production Ready](/install/production/)** - настройка для промышленной эксплуатации
2. **[Безопасность](/configure/security/)** - настройка HTTPS, авторизации и аудита
