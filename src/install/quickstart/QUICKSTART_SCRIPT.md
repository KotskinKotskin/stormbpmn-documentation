...
5. Убедитесь, что установлен согласованный с менеджером образ с поддержкой v2. Откройте
   [http://localhost:8081/admin-onboarding](http://localhost:8081/admin-onboarding),
   скопируйте **весь код активации без сокращения** и передайте его менеджеру.

6. Получив от менеджера отдельный ключ v2 для этой инсталляции, добавьте его в приложение.
   Для тестового контура, DR-площадки и каждого клона запросите отдельный ключ.

- Зайдите в Portainer [http://localhost:9080](http://localhost:9080) с учетными данными из таблицы выше
...
- Дождитесь перезапуска приложения и проверьте лицензию на странице
  ...
  завершите мастер настройки и проверьте вход лицензированного обычного пользователя и
  необходимые модули, включая DES. До активации не открывайте установку пользователям:
  пустой v2 выбирает legacy-путь и не гарантирует блокировку при отсутствии ключей.
  Порядок первичной активации и восстановления — в [разделе о лицензии](/operation/License.md#активация-инсталляции).
