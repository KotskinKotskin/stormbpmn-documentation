...
Мы используем **`Qwen/Qwen3-Next-80B-A3B-Instruct`** — и это не наугад: мы прогнали **8 моделей через
реальный чат Storm** с настоящими MCP-инструментами на четырёх типах задач. Подробный разбор с
бенчмарком, таблицей кандидатов под 96 ГБ и оценками скорости — в отдельной статье
**[Локальная LLM для AI-чата](../operation/LocalLLM.md)**. Кратко, почему победила именно она:
...
- **Прошла оба главных фильтра** — собрала крупный процесс (20–30 элементов) И удержала контекст в
  многоходовом чате (где половина моделей ломается, выдавая вызовы тулов текстом). См. бенчмарк по ссылке.
- **MoE-архитектура A3B** (Active 3B) — из 80B на токен активны ~3B, поэтому генерация быстрая.
