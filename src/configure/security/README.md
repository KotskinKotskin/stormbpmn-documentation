...
| **assm.app.modulbank.token** | Токен для проверки подписи веб-хуков Модульбанка | Токен из личного кабинета Модульбанка | (пусто) |
...
::: warning Безопасность платежей Модульбанк

Если переменная **assm.app.modulbank.token** не задана, проверка подписи отключена (fail-open). Это означает, что система примет любое уведомление от Модульбанка без проверки подлинности, что может привести к фальшивым оплатам. Для production обязательно укажите токен.
...
### Защита API и артефактов

- [ ] **Swagger UI защищен** - включен Basic Auth для доступа к документации API
- [ ] **Секреты исключены из образа** - используется `.dockerignore` для исключения `.env` файлов
- [ ] **Фронтенд собран отдельно** - используется `frontend-dist` вместо исходников для исключения секретов из образа

::: tip Защита Swagger UI
Swagger UI должен быть защищен Basic Auth в production, чтобы предотвратить несанкционированный доступ к документации API и возможным тестовым данным.
