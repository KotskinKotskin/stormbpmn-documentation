...
::: tip Сессия StormBPMN может жить ровно столько, сколько сессия у провайдера
...
Подробности и порядок включения — [Привязка сессии к SSO](./sso-session-binding.md).
:::
...
::: tip СУДИР
Для СУДИР достаточно стандартного разрешения `profile` — оно уже отдаёт `family_name` (фамилия), `name` (имя) и `middle_name` (отчество) в ответе `/oauth/me`. Дополнительных доработок или специальных прав на стороне СУДИР не требуется: укажите `/oauth/me` в **OAuthUserInfoUri**, и ФИО заполнится автоматически.
:::
