...
| **adminTeamMaxMembers** | Лимит участников админской команды (fullAccess). Значение должно быть >= 1. По умолчанию: 2. Можно переопределить через переменную окружения `STORM_ADMIN_TEAM_MAX_MEMBERS` | По требованию безопасности |
